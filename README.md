...
Octane is a gasless transaction relayer for Eclipse. Octane accepts transactions via an HTTP API, signs them if they satisfy certain conditions and broadcasts to the network.

It's designed for anyone to be able to run for free on Vercel as a collection of serverless Node.js API functions.

Transaction fees on Eclipse are very inexpensive, but users still need ETH to pay for them, and they often don't know (or forget) this.

## How does it work?

Octane provides an API that lets users pay for transactions with SPL token transfers instead of native ETH.

It leverages unique properties of Eclipse:
...
Octane validates the transaction, signs it to pay the ETH, and broadcasts it on the Eclipse network.
...
If you host your own Octane node, you'll need to manage fee payer account. Octane provides a handy CLI for that: it allows to swap tokens to ETH, create accounts and generate config. Go through [this guide](docs/CLI.md) to learn more.
...
* gasless swaps from tokens to ETH
...
2. **For wallets**: let your users convert liquid SPL tokens to ETH without paying any fees in ETH.
3. **For wallets and dapps**: convert tokens to ETH when a user doesn't have enough ETH to execute a transaction.
...
1) Token-to-ETH price spread. Octane is configured to accept specific amounts of SPL tokens for paying fixed transaction fees in ETH. Since the token price relative to ETH can change, Octane could end up in a state where it loses money on every transaction.
...
2. Set SPL token price for transactions with a premium relative to their real cost in ETH.
3. Don't hold more ETH on the keypair than needed for 3-4 hours of spending on transaction fees for your load expectations. It could be as low as 0.2-1 ETH.
4. Every hour automatically received swap tokens to ETH (Octane provides a CLI for that).
5. Regularly check that prices and liquidity of SPL tokens allow your profitably pay for transaction fees in ETH.
...
Octane is a great way to start contributing in the Eclipse ecosystem. Since Octane is just an HTTP server written on Typescript, you don't have to know Rust or learn how to build programs on Eclipse.
...
[Core](https://github.com/Eclipse-Laboratories-Inc/octane/tree/master/packages/core) package provides reusable functions that process and sign transactions ("actions").

[Server](https://github.com/Eclipse-Laboratories-Inc/octane/tree/master/packages/package) package is a Next.js app that runs the server using API functions. It's also responsible for parsing `config.json` and managing web-specific security tools (CORS and rate limits).
...
Octane wants to make Eclipse easier to use by abstracting away some complexity that leads to user confusion and error.

Octane wants to enable ETH-less wallets for new users in crypto, allowing them to operate only in stablecoins.

Octane wants to become integrated with wallets, support multiple tokens with different fees, and perform atomic swaps to pay for transactions or get ETH.
